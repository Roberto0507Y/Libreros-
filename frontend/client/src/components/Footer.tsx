...
const socialLinks = [
  ...
  },
  {
    ...
    icon: (
      <svg viewBox="0 0 30 30" fill="currentColor" className="h-4.5 w-4.5">
...
export const Footer = ({ onAction }: FooterProps) => {
  return (
    <footer
      className="relative mt-16 overflow-hidden bg-[linear-gradient(180deg,#09132d_0%,#081126_100%)] text-white"
      id="footer-root"
    >
      <div className="pointer-events-none absolute inset-0 bg-[radial-gradient(circle_at_18%_28%,rgba(56,189,248,0.14),transparent_24%),radial-gradient(circle_at_52%_84%,rgba(14,165,233,0.12),transparent_18%),radial-gradient(circle_at_84%_20%,rgba(37,99,235,0.14),transparent_24%)]" />

      <div className="relative mx-auto w-full max-w-7xl px-4 py-12 sm:px-6 lg:px-8">
        <div className="px-2 py-3 lg:px-4">
          <div className="grid gap-10 md:grid-cols-2 xl:grid-cols-[1.15fr_1fr_0.95fr] xl:gap-x-12">
            <div className="flex flex-col items-center text-center md:items-start md:text-left">
              <div className="flex items-center gap-4">
                <div className="grid h-18 w-18 place-items-center rounded-[24px] border border-cyan-300/45 bg-[linear-gradient(135deg,rgba(9,20,46,0.92),rgba(10,34,76,0.88))] shadow-[0_14px_34px_rgba(0,0,0,0.28)]">
                  <img
                    alt="Librería Digital Nexus"
                    className="h-11 w-auto object-contain drop-shadow-[0_10px_18px_rgba(37,99,235,0.22)]"
                    src="/libreria.png"
                  />
                </div>
                <div>
                  <p className="text-left text-[11px] font-semibold uppercase tracking-[0.24em] text-sky-200/80">
                    Librería Digital
                  </p>
                  <h3 className="text-left text-[28px] font-extrabold tracking-[-0.04em] text-white">
                    Librería Digital Nexus
                  </h3>
                </div>
              </div>

              <p className="mt-8 max-w-xl text-[17px] leading-10 text-slate-200/88">
                Compra productos escolares, de oficina y tecnología en un solo lugar.
              </p>
              ...
              <div className="mt-8 flex flex-wrap items-center justify-center gap-3 md:justify-start">
                {socialLinks.map((link) => (
                  <a
                    ...
                    className="inline-flex h-13 w-13 items-center justify-center rounded-full border border-white/12 bg-white/[0.04] text-slate-200/88 transition hover:-translate-y-0.5 hover:border-sky-300/30 hover:bg-white/[0.08] hover:text-white"
                    href={link.href}
                    rel="noreferrer"
                    target="_blank"
                  >
            ...
            <div className="grid gap-8 sm:grid-cols-2">
              {footerSections
                .filter((section) => section.title === 'Comprar' || section.title === 'Cuenta')
                .map((section) => (
                  <div
            ...
            <div className="flex flex-col items-center text-center md:items-start md:text-left">
              ...
              <div className="mt-5 space-y-3 text-sm leading-7 text-slate-200/84" id="footer-contacto">
                <p>Guatemala</p>
              ...
              <div className="mt-6 rounded-[22px] border border-white/10 bg-white/[0.04] px-4 py-4 text-sm leading-7 text-slate-100/88">
                Compra segura · Entrega confiable · Atención personalizada
              </div>
            </div>
          </div>

          <div className="my-4 mt-12 h-px w-full bg-gradient-to-r from-transparent via-white/14 to-transparent" />

          <div className="flex flex-col items-center justify-between gap-4 md:flex-row">
            <p className="text-sm text-white/60">© 2026 Librería Digital Nexus. Todos los derechos reservados.</p>
